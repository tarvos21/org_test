...
- MUST, adding one space before an open bracket and after an close bracket, same as a word, if a comma follows a closed bracket, then no space is needed
- RECOMMEND, not adding spaces around a hyphen, sometimes it is necessary to fine-tune the letter-spacing
- RECOMMEND, using slash to locate two parallel things like "A or B", without spaces around in principle

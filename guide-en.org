...
This guide will systematically explain the essentials of resume writing and typesetting with particular emphasis on typography, helping you make a resume pretty both in form and spirit. For each topic, we will first explain WHAT it is, then WHY is it so, and finally How to do it.

The guide is trying to be as concise and clear as possible, for different items there are four levels of priority:
...
- Caution, require advanced knowledge or features in typesetting, or diverse in different languages and cultures, or varies with regulation in companies, dangerous to use if not informed 
...
According to whether or not the widths of characters are the same, the font can be divided into monospaced font and proportional font. As the name suggests, all letters in monospaced font occupy the same amount of horizontal space, while in proportional font they don't.
...
In traditional printing, proportional fonts can improve the legibility of words. Due to technical limitations, the early computers and typewriters can not adjust the width of characters. So all characters are made into the same width, thus monospaced fonts come into being. With the improvement of computer technology, the GUI(graphical user interface) has become the mainstream and develops continuously; technical limitations no longer exist, so the proportional typeface has become quite popular, since it makes more natural readability for human.
